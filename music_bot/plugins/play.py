...
# Har group ka state store karne ke liye
# queue: [(url, title), ...]
# loop: "off" | "song" | "queue"
...
def register(bot, call_py):
    ...
    async def play_cmd(client, message):
        ...
        try:
            if state["playing"]:
                ...
                await call_py.play(chat_id, MediaStream(first_url))
    ...
    @call_py.on_update()
    async def on_call_update(client, update):
        # Naye py-tgcalls me stream-end update ka type check karke agla gaana chalate hain
        update_name = type(update).__name__
        if "StreamEnd" in update_name or "Ended" in update_name:
